...
def todict(obj, ctx=None):
    ...
    if isinstance(obj, dict):
        ...
        return [todict(v, ctx) for v in obj]
    elif hasattr(obj, "__dict__"):
        data = dict([(key, todict(value, ctx))
                     for key, value in obj.__dict__.items()
                     if not callable(value) and not key.startswith('_')])
        if ctx is not None and hasattr(obj, "__class__"):
            data[ctx] = obj.__class__.__name__
        return data
    else:
        return obj
